...
export const ClubStripeConnect: React.FC<ClubStripeConnectProps> = ({
  ...
  useEffect(() => {
    ...
    loadStatus();
  }, [stripeData?.stripeConnectedAccountId, clubId]);
  ...
  const isReady = status && isClubStripeReady(status);
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="bg-gray-900 rounded-lg p-3">
              ...
              <div className={`font-medium ${status?.chargesEnabled ? 'text-green-400' : 'text-red-400'}`}>
                {status?.chargesEnabled ? '✓ Enabled' : '✗ Disabled'}
              </div>
            </div>
            <div className="bg-gray-900 rounded-lg p-3">
              <div className="text-gray-400">Payouts</div>
              <div className={`font-medium ${status?.payoutsEnabled ? 'text-green-400' : 'text-red-400'}`}>
