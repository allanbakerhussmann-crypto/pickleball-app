 ...
 * FIXED: Input focus issue by rendering steps inline instead of as nested components
 * 
...
export const CreateLeague: React.FC<CreateLeagueProps> = ({ onBack, onCreated }) => {
  ...
  // ============================================
  // RENDER STEP CONTENT - INLINE (fixes focus issue)
  // ============================================
  ...
  const renderStepContent = () => {
    switch (step) {
                ...
                {divs.map(d => (
                  <div key={d.id} className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                    ...
                    <div className="grid grid-cols-4 gap-2 text-sm">
                      <div>
                        <label className="block text-xs text-gray-500">Min Rating</label>
                        <input type="number" step="0.1" value={d.minRating || ''} onChange={e => updDiv(d.id, { minRating: e.target.value ? parseFloat(e.target.value) : null })} className="w-full bg-gray-900 text-white p-1.5 rounded border border-gray-600"/>
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500">Max Rating</label>
                        <input type="number" step="0.1" value={d.maxRating || ''} onChange={e => updDiv(d.id, { maxRating: e.target.value ? parseFloat(e.target.value) : null })} className="w-full bg-gray-900 text-white p-1.5 rounded border border-gray-600"/>
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500">Min Age</label>
                        <input type="number" value={d.minAge || ''} onChange={e => updDiv(d.id, { minAge: e.target.value ? parseInt(e.target.value) : null })} className="w-full bg-gray-900 text-white p-1.5 rounded border border-gray-600"/>
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500">Max</label>
                        <input type="number" value={d.maxParticipants || ''} onChange={e => updDiv(d.id, { maxParticipants: e.target.value ? parseInt(e.target.value) : null })} className="w-full bg-gray-900 text-white p-1.5 rounded border border-gray-600"/>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      case 4:
        if (!isDoubles) {
          return <div className="text-center py-8 text-gray-400">Partner settings for doubles only</div>;
        }
        return (
          <div className="space-y-4">
                ...
                {(['registration_close', 'season_start', 'anytime', 'specific_week'] as const).map(r => (
                  <label key={r} className={`flex items-center gap-2 p-2 rounded cursor-pointer ${partner.partnerLockRule === r ? 'bg-gray-700' : ''}`}>
                    ...
                    <span className="text-white text-sm">
                      {r === 'registration_close' ? 'When registration closes' : r === 'season_start' ? 'When season starts' : r === 'anytime' ? 'Can change anytime' : 'After specific week'}
                    </span>
        ...
        return (
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-5 gap-2">
                {[{ k: 'pointsForWin', l: 'Win' }, { k: 'pointsForDraw', l: 'Draw' }, { k: 'pointsForLoss', l: 'Loss' }, { k: 'pointsForForfeit', l: 'Forfeit' }, { k: 'pointsForNoShow', l: 'No-Show' }].map(p => (
                  <div key={p.k}>
                    ...
                    <input type="number" value={(scoring as any)[p.k]} onChange={e => setScoring({ ...scoring, [p.k]: parseInt(e.target.value) || 0 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600 text-center"/>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select value={matchFmt.bestOf} onChange={e => setMatchFmt({ ...matchFmt, bestOf: parseInt(e.target.value) as 1|3|5 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600">
                    <option value={1}>1 game</option><option value={3}>3 games</option><option value={5}>5 games</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-gray-500">Points/Game</label>
                  <select value={matchFmt.gamesTo} onChange={e => setMatchFmt({ ...matchFmt, gamesTo: parseInt(e.target.value) as 11|15|21 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600">
                ...
                <div>
                  ...
                  <select value={matchFmt.winBy} onChange={e => setMatchFmt({ ...matchFmt, winBy: parseInt(e.target.value) as 1|2 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600">
            ...
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select value={scoreEntry.entryPermission} onChange={e => setScoreEntry({ ...scoreEntry, entryPermission: e.target.value as ScoreEntryPermission })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600">
                ...
                <div>
                  ...
                  <select value={scoreEntry.detailLevel} onChange={e => setScoreEntry({ ...scoreEntry, detailLevel: e.target.value as ScoreDetailLevel })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600">
                ...
                <div>
                  ...
                  <select value={scoreEntry.confirmationRequired} onChange={e => setScoreEntry({ ...scoreEntry, confirmationRequired: e.target.value as ScoreConfirmation })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600">
            ...
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
              ...
              <select value={scoreEntry.disputeResolution} onChange={e => setScoreEntry({ ...scoreEntry, disputeResolution: e.target.value as DisputeResolution })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600">
              ...
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" value={challenge.challengeRange} onChange={e => setChallenge({ ...challenge, challengeRange: parseInt(e.target.value) || 3 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" min={1} max={10}/>
                  ...
                  <div>
                    ...
                    <input type="number" value={challenge.responseDeadlineHours} onChange={e => setChallenge({ ...challenge, responseDeadlineHours: parseInt(e.target.value) || 48 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600"/>
                  </div>
                </div>
              </div>
            )}
            ...
            {basic.format === 'round_robin' && (
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                ...
                <div>
                  ...
                  <input type="number" value={rr.rounds} onChange={e => setRr({ ...rr, rounds: parseInt(e.target.value) || 1 })} className="w-24 bg-gray-900 text-white p-2 rounded border border-gray-600" min={1} max={4}/>
                </div>
              </div>
            )}
            ...
            {basic.format === 'swiss' && (
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                ...
                <div>
                  ...
                  <input type="number" value={swiss.rounds} onChange={e => setSwiss({ ...swiss, rounds: parseInt(e.target.value) || 4 })} className="w-24 bg-gray-900 text-white p-2 rounded border border-gray-600" min={2} max={10}/>
                </div>
              </div>
            )}
              ...
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input type="number" value={box.playersPerBox} onChange={e => setBox({ ...box, playersPerBox: parseInt(e.target.value) || 4 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" min={3} max={8}/>
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500">Promote</label>
                    <input type="number" value={box.promotionSpots} onChange={e => setBox({ ...box, promotionSpots: parseInt(e.target.value) || 1 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" min={0} max={3}/>
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500">Relegate</label>
                    <input type="number" value={box.relegationSpots} onChange={e => setBox({ ...box, relegationSpots: parseInt(e.target.value) || 1 })} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" min={0} max={3}/>
                  </div>
                </div>
              </div>
            )}
                    ...
                    <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input type="text" value={fmtCur(price.entryFee)} onChange={e => setPrice({ ...price, entryFee: parseCur(e.target.value) })} className="w-full bg-gray-900 text-white p-2.5 rounded border border-gray-600"/>
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Fee Type</label>
                          <select value={price.entryFeeType} onChange={e => setPrice({ ...price, entryFeeType: e.target.value as 'per_player' | 'per_team' })} className="w-full bg-gray-900 text-white p-2.5 rounded border border-gray-600">
                            <option value="per_player">Per Player</option><option value="per_team">Per Team</option>
                          </select>
                        </div>
                      </div>
                    </div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
          ...
          return (
            <React.Fragment key={n}>
              <div className="flex flex-col items-center min-w-[40px]">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${skip ? 'bg-gray-700 text-gray-500' : active ? 'bg-blue-600 text-white' : done ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
                  {done ? '✓' : n}
                </div>
      ...
      {error && (
        <div className="bg-red-900/20 border border-red-600 rounded-lg p-3 mb-4 text-red-400">{error}</div>
      )}
