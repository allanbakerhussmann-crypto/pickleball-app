...
import { getMeetups, subscribeToTournaments, subscribeToLeagues, subscribeToTeamLeagues, getAllClubs } from '../services/firebase';
...
const HomePage: React.FC = () => {
  ...
  const [teamLeagues, setTeamLeagues] = useState<TeamLeague[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const activeClubs = clubsData
          ...
          .sort((a, b) => (b.members?.length || b.memberCount || 0) - (a.members?.length || a.memberCount || 0))
          .slice(0, 4);
    ...
    // Subscribe to team leagues (V07.54)
    const unsubTeamLeagues = subscribeToTeamLeagues((data) => {
      // Show active and registration open team leagues
      const activeTeamLeagues = data
        .filter(l => l.status === 'active' || l.status === 'registration')
        .slice(0, 4);
      setTeamLeagues(activeTeamLeagues);
    });
              ...
              >
                ...
                <div className="text-sm text-gray-400 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{club.members?.length || club.memberCount || 0} members</span>
                  </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Team Leagues</h2>
              <p className="text-sm text-gray-400">Club vs Club interclub competitions</p>
            </div>
          </div>
          <button
            onClick={() => navigate(ROUTES.TEAM_LEAGUES)}
        ...
        {loading ? (
          ...
          <div className="bg-gray-800/50 rounded-xl p-8 text-center border border-gray-700">
            <p className="text-gray-400">No active team leagues</p>
            {isOrganizer && (
              <button
                onClick={() => navigate(ROUTES.TEAM_LEAGUE_CREATE)}
                className="mt-3 text-amber-400 hover:text-amber-300 text-sm font-semibold"
              >
                Create a Team League →
              </button>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {teamLeagues.map(league => (
              <button
                key={league.id}
                onClick={() => navigate(getRoute.teamLeagueDetail(league.id))}
                className="bg-gray-800 rounded-xl p-4 border border-gray-700 hover:border-amber-600 transition-colors text-left"
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold text-white truncate">{league.name}</h3>
                  <span className={`text-xs px-2 py-1 rounded-full ${
                ...
                <div className="text-sm text-gray-400 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Max {league.maxTeams || '?'} teams</span>
                  </div>
