...
exports.guest_unsubscribe = exports.guest_onGuestCreated = void 0;
...
// =============================================================================
function getEmailKeyForVisitCount(totalVisits) {
    switch (totalVisits) {
...
function buildGuestMarketingEmail(params) {
...
// =============================================================================
exports.guest_onGuestCreated = functions
    ...
    try {
        ...
        if (!guest.email || !guest.email.trim()) {
            console.log(`guest_onGuestCreated: Skipping guest ${context.params.guestId} - no email (cash guest)`);
            return;
        }
        ...
        if (meetupSnap.exists) {
            ...
            meetupTitle = (meetupData === null || meetupData === void 0 ? void 0 : meetupData.title) || 'Unknown Meetup';
            clubId = (meetupData === null || meetupData === void 0 ? void 0 : meetupData.clubId) || '';
        }
        else {
            console.warn(`guest_onGuestCreated: Standing meetup ${standingMeetupId} not found, using defaults`);
        }
        // -----------------------------------------------------------------------
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (profileSnap.exists) {
                ...
                const updatedCashVisits = guest.paymentMethod === 'cash'
                    ? existing.cashVisits + 1
                    : existing.cashVisits;
                const updatedStripeVisits = guest.paymentMethod === 'stripe'
                ...
                // --- CREATE new profile ---
                const newProfile = Object.assign(Object.assign({ email, names: [guest.name.trim()], primaryName: guest.name.trim(), totalVisits: 1, totalSpend: guest.amount, firstVisitAt: guest.createdAt, lastVisitAt: guest.createdAt, recentVisits: [visitSummary], clubIds: clubId ? [clubId] : [], meetupIds: [standingMeetupId], cashVisits: guest.paymentMethod === 'cash' ? 1 : 0, stripeVisits: guest.paymentMethod === 'stripe' ? 1 : 0, conversionStatus: 'guest', emailConsent: hasEmailConsent }, (hasEmailConsent && { emailConsentAt: now })), { unsubscribeToken: crypto.randomUUID(), emailsSent: [], createdAt: now, updatedAt: now });
                transaction.set(profileRef, newProfile);
            }
        });
        console.log(`guest_onGuestCreated: Profile updated for ${email} ` +
        ...
        try {
            ...
            const finalProfile = finalSnap.data();
            if (!finalProfile)
                return;
            if (!finalProfile.emailConsent)
                return;
            if (finalProfile.conversionStatus !== 'guest')
                return;
            const emailKey = getEmailKeyForVisitCount(finalProfile.totalVisits);
            if (!emailKey)
                return;
            if ((finalProfile.emailsSent || []).includes(emailKey))
                return;
            ...
            const emailResult = await (0, comms_1.sendEmail)(finalProfile.email, subject, textBody, htmlBody);
        ...
        console.error(`guest_onGuestCreated: Failed to update profile for guest ${context.params.guestId}:`, error);
    }
});
// =============================================================================
// Helper: Build unsubscribe confirmation HTML page
// =============================================================================
function buildUnsubscribeHtml(success, errorMessage) {
    const icon = success
...
// =============================================================================
exports.guest_unsubscribe = functions
    .region('australia-southeast1')
...
//# sourceMappingURL=guestMarketing.js.map