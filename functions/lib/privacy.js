...
exports.privacy_processRequest = exports.privacy_runDataCleanup = exports.privacy_scheduledDataCleanup = exports.privacy_logBreach = exports.privacy_notifyBreachAffectedUsers = void 0;
 ...
 */
exports.privacy_notifyBreachAffectedUsers = functions.https.onCall(async (data, context) => {
    var _a;
    ...
    if (!breachId) {
        throw new functions.https.HttpsError('invalid-argument', 'Breach ID is required.');
    }
    // Verify caller is an admin
    const callerIsAdmin = await isAppAdmin(callerId);
    if (!callerIsAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only app admins can send breach notifications.');
    }
    // Get breach record
    const breachDoc = await db.collection('security_breaches').doc(breachId).get();
    if (!breachDoc.exists) {
        throw new functions.https.HttpsError('not-found', 'Breach record not found.');
    }
    const breach = breachDoc.data();
    if (breach.usersNotified) {
        throw new functions.https.HttpsError('already-exists', 'Users have already been notified about this breach.');
    }
    const affectedUserIds = breach.affectedUserIds || [];
    if (affectedUserIds.length === 0) {
        throw new functions.https.HttpsError('failed-precondition', 'No affected users to notify.');
    }
...
- Nature: ${breach.title}
- Data Potentially Affected: ${((_a = breach.dataTypesExposed) === null || _a === void 0 ? void 0 : _a.join(', ')) || 'Account information'}
...
Pickleball Director Security Team
    ...
    usersSnapshot.docs.forEach(userDoc => {
        const userData = userDoc.data();
        if (userData.email) {
            notificationPromises.push(sendEmailNotification(userData.email, subject, baseMessage));
        }
    });
    ...
    await breachDoc.ref.update({
        usersNotified: true,
        actionsToken: admin.firestore.FieldValue.arrayUnion(`[${new Date().toISOString()}] Affected users notified by admin ${callerId}`),
        updatedBy: callerId,
        updatedAt: Date.now(),
    });
    // Log the action
 ...
 */
exports.privacy_logBreach = functions.https.onCall(async (data, context) => {
    var _a, _b, _c;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be logged in to perform this action.');
    }
    ...
    const callerIsAdmin = await isAppAdmin(callerId);
    if (!callerIsAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only app admins can log security breaches.');
    }
    const timestamp = Date.now().toString(36);
    const random = Math.random().toString(36).substring(2, 8);
    const breachId = `BREACH-${timestamp}-${random}`.toUpperCase();
    const now = Date.now();
    const requiresNotification = (data.severity === 'critical' || data.severity === 'high') &&
        ((_b = (_a = data.dataTypesExposed) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0) > 0;
    const breachRecord = {
        id: breachId,
        ...
        estimatedAffectedCount: data.estimatedAffectedCount || ((_c = data.affectedUserIds) === null || _c === void 0 ? void 0 : _c.length) || 0,
 ...
 * Runs daily at 3 AM
 */
exports.privacy_scheduledDataCleanup = functions.pubsub
    .schedule('0 3 * * *')
 ...
 */
exports.privacy_runDataCleanup = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be logged in to perform this action.');
    }
    ...
    const callerIsAdmin = await isAppAdmin(callerId);
    if (!callerIsAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only app admins can run data cleanup.');
    }
    const now = Date.now();
    const dryRun = (_a = data.dryRun) !== null && _a !== void 0 ? _a : true; // Default to dry run for safety
 ...
 */
exports.privacy_processRequest = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be logged in to perform this action.');
    }
    const callerId = context.auth.uid;
    const callerIsAdmin = await isAppAdmin(callerId);
    if (!callerIsAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only app admins can process privacy requests.');
    }
    const { requestId, action, notes } = data;
    const requestDoc = await db.collection('privacy_requests').doc(requestId).get();
    if (!requestDoc.exists) {
        throw new functions.https.HttpsError('not-found', 'Privacy request not found.');
    }
    const now = Date.now();
